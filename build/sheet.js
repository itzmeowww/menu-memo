...
const keys = {
    ...
    client_x509_cert_url: "https://www.googleapis.com/robot/v1/metadata/x509/food-fetch%40food-fetcher-282419.iam.gserviceaccount.com",
};
        ...
        if (!x.properties.title.endsWith("19") && !x.properties.hidden) {
            ...
            data.data.values.forEach((x) => {
                ...
                if (date != "") {
                    ...
                    if (x[1])
                        database[date]["Breakfast"].push(x[1]);
                    if (x[2])
                        database[date]["Lunch"].push(x[2]);
                    if (x[3])
                        database[date]["Dinner"].push(x[3]);
                }
            });
        }
    }
